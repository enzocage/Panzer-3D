...
export class Game {
    ...
    constructor() {
        ...
        this.projectileSpeed = 50;   // Geschwindigkeit der Projektile
    ...
    /**
     * Verarbeitet die Zielauswahl für den Schuss
     */
    handleAiming(targetPoint) {
        if (!this.selectedTank || this.currentGameState !== GAME_STATE.AIMING) return;
        
        const direction = targetPoint.clone().sub(this.selectedTank.mesh.position);
        direction.y = 0; // Schuss nur in horizontaler Ebene
        
        // Berechne Schusswinkel
        const angle = Math.atan2(direction.x, direction.z);
        this.selectedTank.aimTurret(angle);
    ...
    fireProjectile(tank, direction) {
        ...
        projectile.position.y = GAME_CONFIG.TANK_BASE_SIZE.height * 0.8; // Höhe der Kanone
